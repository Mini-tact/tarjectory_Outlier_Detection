...
def Judgement_overlap(L_i, L_j):
    k_1,b_1 = computer_point(L_i[0][0], L_i[0][1], L_i[1][0], L_i[1][1]).define_a_line()
    k_2,b_2= computer_point(L_j[0][0], L_j[0][1], L_j[1][0], L_j[1][1]).define_a_line()
    ...
    if L_j[0][0] > x > L_i[1][0] and L_i[0][1] > y > L_j[0][1]:
        return "disjoint"
    if k_1*L_j[0][0] + b_1==L_j[0][1] and k_1*L_j[1][0] + b_1==L_j[1][1]:
        return "overlap"
    if k_1*L_j[0][0] + b_1==L_j[0][1] or k_1*L_j[1][0] + b_1==L_j[1][1]:
        return "enclose"
...
def lower_bounds_angle(L_i, L_j, fine):
    if minlen(L_i, fine) > minlen(L_j, fine):
        minlen(L_j, fine) * math.sin(Calculate(L_i[0][0],L_i[0][1],L_i[1][0],L_i[1][1]).cos() - max_angle(L_i, fine) - max_angle(L_j, fine))
    else:
        minlen(L_i, fine) * math.sin(Calculate(L_i[0][0],L_i[0][1],L_i[1][0],L_i[1][1]).cos() - max_angle(L_i, fine) - max_angle(L_j, fine))
